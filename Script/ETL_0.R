...
rm(list = ls())
drm <- read_excel("Documentos/Guajira/Muestreo_Guajira_DRM.xls")
#Filtrar campos
...
# Marco geológico
#
ug <- st_read('Documentos/Guajira/UG_250K.shp')
# Proyectar y limpiar
...
# Diagnostico
#
diagnstco<-st_read('Documentos/Guajira/Resultados_Consolidados2023/Diagnostico.shp')
# Filtrar campos
...
# Muestreo FQ
#
muestreo_fq <- read_csv("Documentos/Guajira/Resultados_Consolidados2023/muestreo_fq.csv")
# Filtrar
...
# Microbiologicos
#
micro<-st_read('Documentos/Guajira/Resultados_Consolidados2023/Consolidado_FQ_Microbiologico.shp')
# Filtrar
micro = st_drop_geometry(micro)[,c("ID","fecha","Tipo_P","Conduct","As_ugl","Cd_ugl","Co_ugl","Pb_ugl","F_mgL","SO4_mgL","Na_mgl")]
# Transpoer tabla
micro <- micro %>%
  mutate(across(c(Conduct, As_ugl, Cd_ugl, Co_ugl, Pb_ugl, F_mgL, SO4_mgL, Na_mgl), 
                as.character)) %>%
  pivot_longer(
    cols = c(Conduct, As_ugl, Cd_ugl, Co_ugl, Pb_ugl, F_mgL, SO4_mgL, Na_mgl),
    names_to = "variable",
...
# Escribir 
st_write(micro,'Documentos/Guajira/DRM.gpkg',layer='muestreo_fq')
